...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # TEK-DOSYA: SecretsManager (env → pemf_secrets.json embedded; varsayılan anon key dahil).
        try:
    ...
    if os.environ.get("PEMF_ENABLE_TUNNEL") == "1":
        ...
        try:
            ...
            start_tunnel_watchdog(port=args.port)
    ...
    # periyodik kontrol eder → yeni sürüm varsa UI'da bildirilir (UYGULAMAZ; operatör /api/update/apply
    # ile onaylar → indir+SHA256+aktif-tedavi-yoksa sessiz kur). Tünelden bağımsız çalışır.
    try:
