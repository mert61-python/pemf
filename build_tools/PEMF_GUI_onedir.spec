...
if os.path.exists(version_file):
    app_version = open(version_file, encoding='utf-8').read().strip()
else:
    app_version = '0.0.0'
...
# Config dosyaları - plug-and-play kurulum için gerçek config ve credential
# dosyaları bundle'a dahil edilir. Kullanıcı onayıyla MQTT/e-posta şifreleri de
# paketlenir; ProductionConfigManager ilk açılışta bunlardan APPDATA config'i üretir.
config_dir = os.path.join(project_path, 'config')
if os.path.exists(config_dir):
    config_count = 0
    for root, dirs, files in os.walk(config_dir):
        for file in files:
            if file.endswith(('.json', '.template', '.conf', '.txt', '.h')):
                full_path = os.path.join(root, file)
                rel_dir = os.path.relpath(root, project_path)
                datas.append((full_path, rel_dir))
                config_count += 1
    print(f"[OK] Config klasörü dahil edildi: {config_count} dosya")
else:
    print("⚠ config klasörü bulunamadı; runtime defaultlarına düşülecek.")

data_config = os.path.join(project_path, 'data', 'config.json')
if os.path.exists(data_config):
    datas.append((data_config, 'data'))
    print("[OK] data/config.json dahil edildi")
