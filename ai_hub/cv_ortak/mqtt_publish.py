...
#: Eski brokerlarin SERT siniri. Asilirsa baglanti REDDEDILIR ve yayinci
#: SESSIZCE hic calismaz (backend `estop-cloud` rolunde birebir yasandi).
_MQTT_ID_SINIRI = 23


def benzersiz_client_id(taban: str) -> str:
    """Yapilandirmadan gelen kimlige SUREC eki koyar.

    ⚠️ SABIT client_id OLMAZ (2026-09-19'da A2 birlestirmesi sirasinda olculdu).
    Ayni kimlikle ikinci baglanti brokerda BIRINCIYI DUSURUR. Bu depo ayni
    sinifi backend'de IKI kez yasadi ve `_mqtt_client_id(role)` ile cozdu
    (`pemf_{role}_{pid}`). CV yayincisi o dersten muaf kalmisti: hem kod
    varsayilani hem ornek YAML sabit dize veriyordu ("phantom_cv_pipeline").

    ⚠️ Kesme ONCE yapilir: taban uzunsa pid eki sinira TASIRDI ve kimlik
    reddedilirdi — sessiz yayinci arizasi.
    """
    son_ek = f"_{os.getpid() % 100000}"
    return taban[: _MQTT_ID_SINIRI - len(son_ek)] + son_ek
...
def _build_client(out_cfg: OutputCfg) -> "mqtt.Client":
    ...
    client = mqtt.Client(
        client_id=benzersiz_client_id(out_cfg.mqtt_client_id),
