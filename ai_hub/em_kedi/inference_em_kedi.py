...
"""inference_em_kedi.py — Kedi Inference (Mert pattern + best ONNX).

Best model : BiLSTM_XXL_Raw   (Combined R2 = 0.9481)
Input  (6) : x, y, z, organ_id, achieved_B, duty_sum
        organ_id in {0,1,2,3,4,5,6}
Output (15): D1-D7, P1-P7 (deg, gauge P1=0), result_E (tek E sutunu)

ONNX model raw cikisi: 22-dim
    [D(7) | sin(P)(7) | cos(P)(7) | E(1)]

Kullanim:
  python inference_em_kedi.py --x 78 --y 210 --z -57.85 \
...
class KediPredictor:
    ...
    def _run_onnx(self, X_sc: np.ndarray) -> np.ndarray:
        ...
        n = X_sc.shape[0]
        if y_sc.size == n * 22 and y_sc.shape != (n, 22):
            y_sc = y_sc.reshape(n, 22)
        return y_sc

    def predict(self, x, y, z, organ_id, achieved_B=0.001, duty_sum=2.0):
        X_sc = self._build_input(x, y, z, organ_id, achieved_B, duty_sum)
        y_sc = self._run_onnx(X_sc)
        D, P_deg, E = self._unscale(y_sc)
        out = {}
        for i, col in enumerate(D_COLS):
            out[col] = float(D[0, i])
        for i, col in enumerate(P_COLS):
            out[col] = float(P_deg[0, i])
        out["result_E"] = float(E[0])
        return out
...
def main():
    ...
    print(f"\nKediPredictor ({MODEL_NAME}, Combined_R2=0.9481)")
    pred = KediPredictor()
    if args.csv:
