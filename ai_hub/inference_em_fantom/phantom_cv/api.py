...
Calistir:
  uvicorn phantom_cv.api:app --host 0.0.0.0 --port 8765

Veya config'den okuyarak:
  python -m phantom_cv.api --config config.yaml
...
app = FastAPI(title="phantom_cv", version="0.1.0")
_pipeline: PhantomCvPipeline | None = None


def get_pipeline() -> PhantomCvPipeline:
    global _pipeline
    if _pipeline is None:
        cfg_path = os.environ.get(_CONFIG_ENV)
        _pipeline = PhantomCvPipeline(cfg_path)
    return _pipeline
...
def health() -> dict[str, Any]:
    return {"status": "ok", "service": "phantom_cv"}
