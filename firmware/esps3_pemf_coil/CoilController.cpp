...
/* ============================================================================
 * SYNC PULSE ISR — STM32 Master'dan RISING kenar gelince DDS sıfırlanır
 * ============================================================================
 * Bu fonksiyon hem ESP32-S3 hem ESP8266 için aynıdır.
...
void CoilController::begin() {
    ...
    pinMode(PIN_SYNC_IN, INPUT);
    attachInterrupt(digitalPinToInterrupt(PIN_SYNC_IN), syncPulseISR, RISING);
    LOG_PRINTLN(F("[SYNC] ESP8266 Slave: PIN_SYNC_IN hazir (GPIO7, RISING)"));
...
void CoilController::handleCommand(const ControlCommand& cmd) {
    switch (cmd.type) {
        case CMD_START:
            start(cmd.frequency, cmd.dutyCycle, cmd.durationMinutes, cmd.timestamp);
            break;
        case CMD_STOP:
            stop();
            break;
        case CMD_UPDATE_PARAMS:
            setParams(cmd.frequency, cmd.dutyCycle, cmd.durationMinutes);
            break;
        case CMD_SYNC_ALL:
            if (_pwmActive) {
                stop();
                start(_pwmFrequency, _pwmDutyCycle, _pwmDurationMinutes, cmd.timestamp);
            } else {
...
void CoilController::start(int freq, int duty, int durationMin, unsigned long long targetTimestampMs) {
    _pwmFrequency = constrain(freq, 1, 1000);  // 1Hz - 1kHz
    _pwmDutyCycle = constrain(duty, 1, 99);     // 1% - 99% (tutarlilik icin setParams() ile ayni)
    _pwmDuration = durationMin * 60 * 1000;  // dakikayi ms'ye cevir
    _pwmDurationMinutes = durationMin;  // Dakika olarak sakla
...
void CoilController::setParams(int freq, int duty, int durationMin) {
    _pwmFrequency = constrain(freq, 1, 1000);
    _pwmDutyCycle = constrain(duty, 1, 99);
    _pwmDuration = durationMin * 60 * 1000;
...
void CoilController::populateStatus(StatusData& data) {
    ...
    data.pwm_duty_cycle = _pwmDutyCycle;
    data.pwm_start_timestamp = _pwmStartTimestamp;
    data.pwm_duration = _pwmDurationMinutes;
