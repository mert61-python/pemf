...
def test_KRITIK_cikti_girdinin_icindeyse_ACIK_hata(cli, tmp_path):
    ...
    # dosya-girdisinde ebeveyn klasör de korunur (tarama ebeveyni gezer)
    (girdi / "a.csv").write_text("x", encoding="utf-8")
    with pytest.raises(SystemExit, match="kendi çıktısını"):
        cli._cikti_bekcisi(girdi / "a.csv", girdi / "out")


def test_KARSIT_KANIT_disaridaki_cikti_GECER(cli, tmp_path):
    girdi = tmp_path / "fotolar"
    girdi.mkdir()
