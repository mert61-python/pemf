...
def test_KRITIK_eski_tibbi_kayit_TASINIR(ortam, monkeypatch):
    ...
    (eski / "pemf_treatment_history.db").write_bytes(b"SEANS-GECMISI")
    (eski / "pemf_patients.db").write_bytes(b"HASTALAR")

    _goc(monkeypatch, eski_kok, hedef)

    assert (hedef / "pemf_treatment_history.db").read_bytes() == b"SEANS-GECMISI", (
        "seans gecmisi tasinmadi — klinik BOS gorunurdu"
    )
    assert (hedef / "pemf_patients.db").read_bytes() == b"HASTALAR"
...
def test_KRITIK_kaynak_SILINMEZ(ortam, monkeypatch):
    ...
    eski_kok, eski, hedef = ortam
    (eski / "pemf_treatment_history.db").write_bytes(b"X")
    _goc(monkeypatch, eski_kok, hedef)
...
def test_KRITIK_hedefteki_kayit_EZILMEZ(ortam, monkeypatch):
    ...
    eski_kok, eski, hedef = ortam
    (eski / "pemf_treatment_history.db").write_bytes(b"ESKI")
    (hedef / "pemf_treatment_history.db").write_bytes(b"CALISAN-KLINIK")

    _goc(monkeypatch, eski_kok, hedef)

    assert (hedef / "pemf_treatment_history.db").read_bytes() == b"CALISAN-KLINIK", (
...
def test_ayni_dizin_ise_hicbir_sey_yapmaz(tmp_path, monkeypatch):
    ...
    (d / "pemf_patients.db").write_bytes(b"A")
    _goc(monkeypatch, kok, d)
    assert (d / "pemf_patients.db").read_bytes() == b"A"
...
def test_KRITIK_goc_hatasi_BACKENDI_DUSURMEZ(ortam, monkeypatch):
    ...
    eski_kok, eski, hedef = ortam
    (eski / "pemf_patients.db").write_bytes(b"X")
