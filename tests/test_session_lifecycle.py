...
def _payload(api, **kw):
    base = dict(coil_ids=[6, 7, 8], mode="Manuel", operator_name="op",
                frequency=50, duty=25, intensity=2.0, phase=0, duration_minutes=20)
    base.update(kw)
...
def test_start_session_populates_active_session(api):
    resp = asyncio.run(api.start_session(_payload(api)))
...
def test_start_session_rejects_double_start(api):
    asyncio.run(api.start_session(_payload(api)))
...
def test_start_ai_session_takes_over_active_manual(api):
    ...
    with api._session_lock:
        ...
        api._active_session.update({
            "is_active": True, "session_id": "react_x", "mode": "Manuel",
            "coil_ids": [6, 7, 8], "db_session_id": None, "start_time": 1.0,
        })
