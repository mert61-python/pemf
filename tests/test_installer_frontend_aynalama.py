...
`build_tools/build_installer.ps1` içinde:
    ...
    $FrontendDir = Join-Path $ProjectRoot "apps/ui"     # (satır ~252)
    ...
    $PfDir       = Join-Path $ProjectRoot "apps/ui"     # (satır ~263)  ← AYNI DİZİN

ve aşağıdaki aynalama adımı hâlâ "apps/ui\dist → frontend\dist" varsayımıyla yazılmış:

    $FrontendDistDir = Join-Path $FrontendDir "dist"          # = apps/ui\dist
...
Sonuç: taze export edilen `apps/ui\dist` silinir, kopyalama `ItemNotFoundException` atar ve
`$ErrorActionPreference = "Stop"` yüzünden **offline Inno installer varsayılan yolda hiç
derlenemez**. Yalnız `PEMF_SKIP_FRONTEND=1` ile derlenebiliyor. Ayrıca `apps/ui\dist` silinmiş kaldığı
...
`apps/ui/dist`'ten paketliyor (yayınlanan `base-app.zip` bununla doğrulandı: `frontend\dist\version.json`
diskte VAR ama zip'te YOK → spec `apps/ui/dist` okuyor). Doğru düzeltme kopyalamayı "kaynak == hedef"
durumunda ATLAMAK.

⚠️ NEDEN GREP DEĞİL: bu test bloğu METİN olarak denetlemiyor, betikten çıkarıp **gerçekten
çalıştırıyor** ve `apps/ui\dist`'in hayatta kaldığını ölçüyor. Kaynak-metin denetimleri bu depoda bir
...
def test_KRITIK_aynalama_pf_dist_i_SILMEZ():
    """Blok gerçekten koşturulur; `apps/ui\\dist` ve içindeki export hayatta kalmalı."""
    blok = _aynalama_blogu()

    with tempfile.TemporaryDirectory() as gecici:
        g = Path(gecici)
        pf_dist = g / "apps" / "ui" / "dist"
    ...
    assert hayatta, (
        "aynalama adimi taze export edilen `apps/ui\\dist`'i SILDI → Copy-Item kaynagi bulamaz, "
...
def test_kaynak_ve_hedef_FARKLIYSA_aynalama_hala_calisir_karsit_kanit():
    ...
    with tempfile.TemporaryDirectory() as gecici:
        g = Path(gecici)
        (g / "apps" / "ui" / "dist").mkdir(parents=True)
        (g / "apps" / "ui" / "dist" / "index.html").write_text("<html>EXPORT</html>", encoding="utf-8")
        ...
        kaynak_duruyor = (g / "apps" / "ui" / "dist" / "index.html").exists()
