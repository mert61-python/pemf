...
def generate_ecg_signal(
    duration: float = 10.0, sampling_rate: int = 250, heart_rate: float = 100.0, noise_level: float = 0.1
) -> np.ndarray:
    """
...
def generate_predictor_data(n_samples: int = 500) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    feature_cols = (
        ...
        + [
            f'{sensor}_{stat}'
            for sensor in ['mag_field', 'temp', 'current']
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='data/test_data', help='Output directory for generated data')
    parser.add_argument('--n_ecg', type=int, default=500, help='Number of ECG signals to generate')
    parser.add_argument('--n_predictor', type=int, default=500, help='Number of predictor samples to generate')
    parser.add_argument('--n_monitor', type=int, default=200, help='Number of monitor sequences to generate')
