...
class EmailPDFDialog(QDialog, StyleMixin):
    ...
    def setup_ui(self):
        ...
        recipient_help.setStyleSheet("color: #aaa; font-size: 10px;")
        ...
        subject_help.setStyleSheet("color: #aaa; font-size: 10px;")
        ...
        message_help.setStyleSheet("color: #aaa; font-size: 10px;")
        ...
        gmail_warning.setStyleSheet(
            "color: #ffc107; font-size: 10px; padding: 5px; "
        ...
        shortcut_label.setStyleSheet("color: #888; font-size: 9px;")
        ...
        if self.email_thread and self.email_thread.isRunning():
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                if not self.email_thread.wait(5000):
                    # Timeout olursa zorla sonlandır (son çare)
                    self.logger.warning("Email thread graceful shutdown timeout, forcing terminate")
                    self.email_thread.terminate()
                    self.email_thread.wait(1000)
            else:
                event.ignore()
                return
    ...
    def _load_email_credentials(self):
        ...
        try:
            ...
            self.logger.info(f"Email credentials loaded from config.json")
