...
class EmailPDFDialog(QDialog, StyleMixin):
    ...
    def send_email(self):
        ...
        if not self.gmail_email or not self.gmail_password:
            QMessageBox.warning(
                self,
                "E-posta Ayarı Eksik",
                "Gönderen e-posta bilgileri tanımlı değil.\n\n"
                "PEMF_GMAIL_EMAIL ve PEMF_GMAIL_APP_PASSWORD ortam değişkenlerini "
                "veya Windows Credential Manager kayıtlarını tanımlayın.",
                QMessageBox.StandardButton.Ok
            )
            return
    ...
    def _load_email_credentials(self):
        ...
        try:
            ...
            # Config dosyasının konumunu belirle. PyInstaller onedir içinde
            # dosyalar çoğunlukla _internal/config altında, development'ta ise
            # proje kökü/config altında bulunur.
            current_dir = os.path.dirname(os.path.abspath(__file__))
            parent_dir = os.path.dirname(current_dir)
            candidate_paths = [
            ...
            if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
                candidate_paths.insert(0, os.path.join(sys._MEIPASS, 'config', 'config.json'))

            try:
                from utils.production_config_manager import get_production_config
                candidate_paths.append(str(get_production_config().get_user_config_path()))
            except Exception:
                pass

            config_path = next(
                (os.path.abspath(path) for path in candidate_paths if os.path.exists(path)),
                os.path.abspath(candidate_paths[0])
            )
            
            if not os.path.exists(config_path):
                self.logger.warning(
                    "Email credentials are not configured; keyring/env/config.json not found."
                )
                return "", ""
            ...
            if not gmail_email or not gmail_password:
                self.logger.warning(
                    "config.json email credentials are incomplete; email sending disabled."
                )
                return "", ""
