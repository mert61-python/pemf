...
class UnifiedControlWindow(QMainWindow, HardwareAwareMixin, StyleMixin):
    ...
    _safe_stop_all_signal = pyqtSignal()  # React/API emergency stop -> ana thread hardware stop
    ...
    def __init__(self, main_window=None):
        ...
        self.pending_commands = {}  # {command_id: {'coil_num', 'command', 'timestamp', 'retry_count', 'transport'}}
        ...
        self._safe_stop_all_signal.connect(
            lambda: self._force_stop_all_hardware("safe_stop_signal"),
            Qt.ConnectionType.QueuedConnection
        )
    ...
    def _create_automatic_tab(self):
        ...
        self.auto_frequency_spin.setRange(FREQ_MIN_HZ, FREQ_MAX_HZ)
    ...
    def _create_manual_tab(self):
        ...
        self.master_freq_spin.setRange(int(FREQ_MIN_HZ), int(FREQ_MAX_HZ))
        ...
        self.master_duty_spin.setRange(0.0, 999.0)
        self.master_duty_spin.setValue(25.0)
        ...
        self.start_all_btn.clicked.connect(lambda _: self.start_all_coils())
    ...
    def _create_coil_control(self, parent_layout, coil_num):
        ...
        freq_spin.setRange(int(FREQ_MIN_HZ), int(FREQ_MAX_HZ))
        ...
        duty_spin.setRange(0.0, 999.0)
        duty_spin.setValue(25.0)
        duty_spin.setSuffix(" %")
...
# --- 2. ÇIKTI TABLOSU VE DIAGNOSTİK ---
    ...
    def _on_camera_prediction_ready(self, x, y, z, duties, phases, e_field):
        ...
        for i in range(num_coils_to_update):
            ...
            if duty_val > AI_PRO_DUTY_MAX_RATIO * 100.0:
                duty_val = AI_PRO_DUTY_MAX_RATIO * 100.0
        ...
        if self.ai_pro_tracking_active:
            ...
            # [FIX] Manuel sekmedeki spinbox'ları değil, AI'dan gelen güncel frekans ve süreyi kullan!
            ai_freq = getattr(self, 'current_ai_freq', 1.0)
            ai_dur = getattr(self, 'current_ai_duration', 20)
            
            for i in range(1, 9):
                freqs.append(float(ai_freq))
                durations.append(int(ai_dur))
            self._send_udp_live_update(duties, phases, freqs, durations)

    def _send_udp_live_update(self, duties, phases, freqs=None, durations=None):
        """
        ...
        for i, (duty_ratio, phase_val) in enumerate(zip(duties, phases)):
            stm_duty_ratio = normalize_ai_pro_duty_ratio(duty_ratio)
            udp_duty_ratio = normalize_esp_live_duty_ratio(stm_duty_ratio)
            phase_val = normalize_phase_deg(phase_val)
            final_duties_u16.append(int(round(udp_duty_ratio * 65535)) % 65536)
            final_phases_u16.append(int(round(phase_val * 65535 / 360.0)) % 65536)
            if i < 5:
                stm32_duties.append(stm_duty_ratio)
        ...
        ref_ms = int(_t.monotonic() * 1000) % 1000
        
        # Frekans ve süre değerleri
        stm32_freqs = []
        stm32_durs = []
        if freqs is None or durations is None:
            ai_freq = 1.0  # 1Hz DDS
            ai_dur = self.spin_ai_pro_duration.value() if hasattr(self, 'spin_ai_pro_duration') else 20
            stm32_freqs = [ai_freq] * 5
            stm32_durs = [ai_dur] * 5
        else:
            stm32_freqs = [normalize_frequency_hz(v) for v in freqs[:5]]
            stm32_durs = [normalize_duration_minutes(v) for v in durations[:5]]
            while len(stm32_freqs) < 5: stm32_freqs.append(1.0)
            while len(stm32_durs) < 5: stm32_durs.append(0)

        import zlib
        stm_fmt = '<BB 5f 5f 5f 5I H'
        stm_data = struct.pack(
            stm_fmt,
            0xAA, 0x55,
            *[float(v) for v in stm32_duties],
            *[float(v) for v in stm32_phases],
            *[float(v) for v in stm32_freqs],
            *[int(v) for v in stm32_durs],
            ref_ms,
        )
        stm_msg = stm_data + struct.pack('<I', zlib.crc32(stm_data) & 0xFFFFFFFF)
    ...
    def _send_stm_manual_update(self):
        ...
        # STM32 bağlı değilse kuyrukta birikmeyi önle
        if not getattr(self, 'stm_is_connected', False):
            self.logger.debug("[STM32] _send_stm_manual_update: STM bağlı değil, paket gönderilmedi.")
            return
        ...
        for i in range(1, 6): # Sadece Bobin 1-5
            if i in self.coil_controls:
                ...
                # GUI buton durumu yerine gerçek PWM state'ini kullan (start_btn.isEnabled()
                # sync_waiting gibi ara durumlarda yanıltıcı sonuç verir)
                with self.pwm_status_lock:
                    is_running_ui = self.pwm_status.get(i, {}).get('running', False)
                
                if is_running_ui:
                    duty_val = duty_percent_to_ratio(controls['duty_spin'].value())
                    phase_val = normalize_phase_deg(controls['phase_spin'].value())
                    freq_val = normalize_frequency_hz(controls['freq_spin'].value())
                    dur_val = normalize_duration_minutes(controls['duration_spin'].value())
                else:
                    duty_val = 0.0
                    phase_val = 0.0
                    freq_val = normalize_frequency_hz(controls['freq_spin'].value()) if 'freq_spin' in controls else 100.0
                    dur_val = 0
    ...
    def _force_stop_all_hardware(self, reason: str = "manual") -> None:
        """
        Production safety stop.

        Bu metod fiziksel donanımı durdurmak için tek merkezdir:
        STM32 tarafına duty=0 paketi MainWindow üzerinden gönderilir, MQTT/ESP
        stop komutları da aynı global stop akışına bırakılır. Session kaydı ya da
        tedavi state'i burada bitirilmez; onu stop_treatment() yönetir.
        """
        reason = str(reason or "unknown")
        try:
            self.logger.warning("FORCE hardware stop requested: %s", reason)
        except Exception:
            pass

        # AI Pro canlı aktarımı kapat; button click/toggle çağırmadan state'i söndür.
        try:
            self.ai_pro_tracking_active = False
            if hasattr(self, '_ai_pro_session_timer') and self._ai_pro_session_timer:
                self._ai_pro_session_timer.stop()
            if getattr(self, 'lbl_ai_pro_countdown', None) is not None:
                self.lbl_ai_pro_countdown.setText("--:--")
            if hasattr(self, 'btn_start_ai_pro') and self.btn_start_ai_pro is not None:
                self.btn_start_ai_pro.setChecked(False)
                self.btn_start_ai_pro.setText("🚀 AI Uygulamayı Başlat (1Hz)")
        except Exception as e:
            self.logger.debug("AI Pro stop UI cleanup skipped: %s", e)

        # Lokal PWM state'i durdu olarak işaretle; sonraki STM snapshot'ları da zero-duty üretir.
        try:
            with self.pwm_status_lock:
                for coil_id in range(1, 9):
                    state = self.pwm_status.setdefault(coil_id, {})
                    state['running'] = False
                    state['duty'] = 0.0
                    state['duration'] = 0
            with self.pwm_remaining_time_lock:
                for coil_id in range(1, 9):
                    self.pwm_remaining_time[coil_id] = None
            for coil_id in range(1, 9):
                self._set_coil_ui_state(coil_id, "stopped")
        except Exception as e:
            self.logger.debug("Force stop UI state cleanup skipped: %s", e)

        sent_via_main = False
        if self.main_window and hasattr(self.main_window, 'send_global_stop_command'):
            try:
                self.main_window.send_global_stop_command()
                sent_via_main = True
            except Exception as e:
                self.logger.error("MainWindow global stop failed during force stop: %s", e, exc_info=True)

        # MainWindow yoksa veya global stop hata verdiyse, eldeki STM kuyruğuna zero-duty snapshot gönder.
        if not sent_via_main:
            try:
                self._send_stm_manual_update()
            except Exception as e:
                self.logger.error("Fallback STM manual stop packet failed: %s", e, exc_info=True)
    ...
    def _reset_all_pwms(self):
        ...
        ai_tracking_was_active = bool(
            hasattr(self, 'btn_start_ai_pro') and self.btn_start_ai_pro.isChecked()
        )
        try:
            coils_stopped_by_stop_all = set(self._get_connected_coils())
        except Exception:
            coils_stopped_by_stop_all = set()

        self._force_stop_all_hardware("reset_all_pwms")

        if ai_tracking_was_active:
            self.btn_start_ai_pro.setChecked(False)
            self._toggle_ai_pro_tracking()
            
        # 2. Stop_all_coils fonksiyonu bağlı olan cihazlara stop gönderir
        if not ai_tracking_was_active:
            self.stop_all_coils()
        
        # 3. Her bobine ayri "stop" komutu gonder -- "start"+duty=0 ESP'de
        #    minimum duty enforcement'i tetikler (0.2% sorun). Stop kesin kapatir.
        for i in range(6, 9):
            if i in coils_stopped_by_stop_all:
                continue
            ...
            if self.main_window and hasattr(self.main_window, 'coil_control_requested'):
                self.main_window.coil_control_requested.emit(i, command)
    ...
    def _toggle_ai_pro_tracking(self):
        if not self.btn_start_ai_pro.isChecked():
            ...
            """)
            self._force_stop_all_hardware("ai_pro_off")
        ...
        for coil_id in range(6, 9):
            start_freq = 1  # ESP bobinleri UDP live-update dinleme hazırlığı için

            if coil_id in self.coil_controls:
                self.coil_controls[coil_id]['freq_spin'].setValue(1)
    ...
    def _on_ai_parameters_updated(self, parameters: dict):
        ...
        # [FIX] AI modunda kullanılmak üzere güncel değerleri kaydet
        self.current_ai_freq = freq
        self.current_ai_duration = duration
    ...
    def stop_treatment(self, stop_reason: str = 'manual_stop'):
        ...
        self._force_stop_all_hardware(f"stop_treatment:{stop_reason}")
                    ...
                    for coil_id in connected_coils:
                        ...
                        with self.pending_commands_lock:
                            self.pending_commands[command_id] = {
                                ...
                                'timestamp': time.time(),
                                'retry_count': 0,
                                'transport': 'stm' if coil_id <= 5 else 'mqtt'
                            }

                        if coil_id > 5 and self.main_window and hasattr(self.main_window, 'coil_control_requested'):
                            self.main_window.coil_control_requested.emit(coil_id, command)
                        elif coil_id > 5:
        ...
        try:
            ...
            with self.pending_commands_lock:
                self.pending_commands[command_id] = {
                    ...
                    'timestamp': time.time(),
                    'retry_count': 0,
                    'transport': 'stm' if coil_num <= 5 else 'mqtt'
                }
            
            # Send command via MainWindow signal (ensures only MainWindow writes to MQTT)
            # This prevents conflicts when multiple windows try to control the same coil
            # STM bobinleri (1-5) için MQTT komutu gönderilmez — STM32 doğrudan
            # _send_stm_manual_update() üzerinden besleniyor
            if coil_num > 5:
                if self.main_window and hasattr(self.main_window, 'coil_control_requested'):
            ...
            # [FIX] _send_stm_manual_update() için optimistik olarak durum 'çalışıyor' yapıldı
            with self.pwm_status_lock:
                if coil_num not in self.pwm_status:
                    self.pwm_status[coil_num] = {}
    ...
    def stop_coil(self, coil_num):
        ...
        try:
            ...
            # Send command via MainWindow signal (ensures only MainWindow writes to MQTT)
            # STM bobinleri (1-5) için MQTT komutu gönderilmez — STM32 doğrudan
            # _send_stm_manual_update() üzerinden durdurma paketi alıyor
            if coil_num > 5:
                if self.main_window and hasattr(self.main_window, 'coil_control_requested'):
                    self.main_window.coil_control_requested.emit(coil_num, command)
                else:
            ...
            # [FIX] _send_stm_manual_update() için optimistik olarak durum 'durdu' yapıldı
            with self.pwm_status_lock:
                if coil_num not in self.pwm_status:
                    self.pwm_status[coil_num] = {}
    ...
    def start_all_coils(self, override_freq: Optional[float] = None, override_duty: Optional[float] = None, override_duration: Optional[int] = None):
        ...
        try:
            ...
            mqtt_connected = False
            if self.main_window and hasattr(self.main_window, 'mqtt_client') and self.main_window.mqtt_client:
                if self.main_window.mqtt_client.is_connected():
                    mqtt_connected = True
            
            stm_connected = getattr(self, 'stm_is_connected', False)
            
            if not mqtt_connected and not stm_connected:
            ...
            for coil_id in connected_coils:
                ...
                # [FIX] _send_stm_manual_update() için optimistik olarak durum 'çalışıyor' yapıldı
                with self.pwm_status_lock:
                    if coil_id not in self.pwm_status:
                        self.pwm_status[coil_id] = {}
                    self.pwm_status[coil_id]['running'] = True
            ...
            esp_commands = [
                (coil_id, command, command_id)
                for coil_id, command, command_id in prepared_commands
                if coil_id > 5
            ]

            if esp_commands and (not self.main_window or not hasattr(self.main_window, 'coil_control_requested')):
                self.show_warning("MainWindow coil control signal not available! ESP bobinlerine komut gönderilemedi.")

            for coil_id, command, command_id in prepared_commands:
                # Bobin 1-5 STM32 üzerinden gider; MQTT sadece ESP bobinleri (6-8) için kullanılır.
                if coil_id > 5 and self.main_window and hasattr(self.main_window, 'coil_control_requested'):
                    self.main_window.coil_control_requested.emit(coil_id, command)
                commands_sent += 1
            ...
            if commands_sent > 0:
                ...
                self.logger.info(
                    f"{commands_sent} bağlı bobin başlatıldı: STM={sum(1 for c in connected_coils if c <= 5)}, "
                    f"MQTT/ESP={sum(1 for c in connected_coils if c > 5)}. "
                    f"Hedef zaman: {target_time_str}, Buffer: {buffer_ms}ms, Gönderim süresi: {send_duration_ms:.1f}ms"
                )
                self.show_success(
    ...
    def stop_all_coils(self):
        ...
        try:
            self._force_stop_all_hardware("stop_all_coils")
            ...
            # Sadece bağlı bobinlere komut gönder
            commands_sent = 0
            esp_signal_missing_warned = False
            for coil_id in connected_coils:
                # Unique command ID oluştur (thread-safe)
                command_id = self._get_next_command_id(coil_id)
                ...
                elif coil_id > 5 and not esp_signal_missing_warned:
                    self.show_warning("MainWindow coil control signal not available! ESP bobinlerine stop gönderilemedi.")
                    esp_signal_missing_warned = True
                commands_sent += 1
                ...
                with self.pwm_status_lock:
                    if coil_id not in self.pwm_status:
                        self.pwm_status[coil_id] = {}
                    self.pwm_status[coil_id]['running'] = False
    ...
    def _update_connection_status_label(self, coil_id, is_connected, show_toast=True):
        ...
        if is_connected:
            if coil_id <= 5:
                label.setText("STM ✓")
                label.setToolTip("STM32 COM10 @ 115200 üzerinden kontrol ediliyor.")
            else:
                label.setText("Bağlı")
            ...
            if coil_id <= 5:
                ...
                label.setToolTip("STM32 COM10 @ 115200 bağlantısı bekleniyor.")
            else:
    ...
    def _safe_on_coil_status_updated(self, coil_id, status_data):
        ...
        try:
            ...
            if coil_id_int in self.coil_controls:
                ...
                # [FIX] Kullanıcının manuel girdiği değerlerin sıfırlanmaması için, 
                # sadece bobin aktif olarak çalışıyorken ve UI kilitli değilken parametre kutularını güncelle.
                if not _ui_locked and is_running:
                    ...
                    if 'phase' in status_data:
                        phase_value = status_data['phase']
                        with self.pwm_status_lock:
                            self.pwm_status[coil_id_int]['phase'] = phase_value
                        # Spinbox'ı güncelle
                        try:
                            controls['phase_spin'].setValue(float(phase_value))
                        except (ValueError, TypeError) as e:
                            self.logger.warning(f"Bobin {coil_id_int} için geçersiz phase değeri: {phase_value}, hata: {e}")
    ...
    def _safe_handle_command_ack(self, coil_num, command_id, success, cmd_info):
        ...
        try:
            if success and cmd_info:
                ...
                self._set_coil_ui_state(coil_num, "error")
                transport = (cmd_info or {}).get('transport', 'mqtt')
                target = "STM" if transport == "stm" else "ESP"
                self.show_error(f"Bobin {coil_num} komutu {target} tarafından reddedildi")
        ...
        try:
            if connected:
                if hasattr(self, 'status_text'):
                    self.status_text.setText("STM Bağlandı - COM10 @ 115200")
                    self.status_text.setStyleSheet("color:#22c55e; font-size: 11pt; font-weight:700;")
                if hasattr(self, 'status_subtext'):
                    self.status_subtext.setText("5 bobin sabit seri hat üzerinden aktif")
                ...
                if hasattr(self, 'status_text'):
                    self.status_text.setText("STM Bağlantısı Yok - COM10")
    ...
    def _on_unified_1hz_tick(self):
        ...
        try:
            ...
            # 6. Send periodic keep-alive manual update to STM32 to prevent watchdog timeout
            # NOT: Sadece en az 1 bobin çalışıyorsa gönder (Gereksiz trafik ve spam logları önlemek için)
            if getattr(self, 'stm_is_connected', False):
                any_coil_running = False
                for i in range(1, 9):
                    with self.pwm_status_lock:
                        if self.pwm_status.get(i, {}).get('running', False):
                            any_coil_running = True
                            break
                if any_coil_running:
                    # [FIX] AI Pro Tracking aktifse manuel update gönderme!
                    # Çünkü AI zaten _send_udp_live_update ile 20Hz'de doğru değerleri yolluyor.
                    # Manuel update gönderilirse, manual sekmedeki 0 değerleri STM'yi durdurur.
                    if not getattr(self, 'ai_pro_tracking_active', False):
                        self._send_stm_manual_update()
            ...
            for i in range(1, 9):
                if i in self.coil_controls:
                    ...
                    if is_running:
                        ...
                        phase = float(self.coil_controls[i]['phase_spin'].value())
                        freq = float(self.coil_controls[i]['freq_spin'].value())
                        duration = int(self.coil_controls[i]['duration_spin'].value())
                    else:
                        duty = 0.0
                        phase = 0.0
                        freq = 1.0
    ...
    def _handle_command_timeout(self, command_id):
        ...
        with self.pending_commands_lock:
            ...
            transport = cmd_info.get('transport', 'mqtt')
        ...
        if retry_count < self.max_command_retries:
            ...
            if transport == 'stm':
                self._send_stm_manual_update()
            elif self.main_window and hasattr(self.main_window, 'coil_control_requested'):
                self.main_window.coil_control_requested.emit(coil_num, command_copy)
            
            self.logger.warning(
                f"Command timeout, retrying ({retry_count + 1}/{self.max_command_retries}): {command_id} transport={transport}"
            )
    ...
    def _ack_stm_snapshot_commands(self, status_by_coil: dict):
        """STM snapshot ACK geldiginde sadece STM transport pending komutlarini temizle."""
        ack_items = []
        with self.pending_commands_lock:
            for command_id, info in list(self.pending_commands.items()):
                if info.get('transport') != 'stm':
                    continue
                coil_num = info.get('coil_num')
                if coil_num in status_by_coil:
                    ack_items.append((coil_num, command_id, info.copy(), status_by_coil[coil_num]))
                    del self.pending_commands[command_id]

        for coil_num, command_id, info, status_data in ack_items:
            command = copy.deepcopy(info.get('command', {}))
            if status_data.get("running"):
                command["command"] = "start"
                command["freq"] = status_data.get("freq", command.get("freq", 0))
                command["duty"] = status_data.get("duty", command.get("duty", 0.0))
                command["duration"] = status_data.get("duration", command.get("duration", 0))
            else:
                command["command"] = "stop"
            self._safe_handle_ack_signal.emit(coil_num, command_id, True, {"command": command, "transport": "stm"})

    def handle_stm_error(self, line: str):
        """STM_NACK/STM_ERR satirlarini pending STM komutlari ve UI ile esle."""
        message = str(line or "").strip()
        failed_items = []
        with self.pending_commands_lock:
            for command_id, info in list(self.pending_commands.items()):
                if info.get('transport') == 'stm':
                    failed_items.append((info.get('coil_num', 0), command_id, info.copy()))
                    del self.pending_commands[command_id]

        for coil_num, command_id, info in failed_items:
            self._safe_handle_ack_signal.emit(coil_num, command_id, False, info)

        self.logger.error("STM command error: %s", message)
        try:
            self._safe_show_toast_signal.emit(f"STM hata: {message}", "error")
        except Exception:
            pass
        ...
        try:
            ...
            with self.pending_commands_lock:
                self.pending_commands[command_id] = {
                    ...
                    'timestamp': time.time(),
                    'retry_count': 0,
                    'transport': 'stm' if int(coil_num) <= 5 else 'mqtt'
                }
    ...
    def closeEvent(self, event):
        ...
        # Production safety: pencere/uygulama kapanırken STM duty=0 paketi kesin kuyruğa girsin.
        try:
            self._force_stop_all_hardware("unified_close")
        except Exception as e:
            self.logger.error("Unified close force stop failed: %s", e, exc_info=True)
        ...
        # Hasta silme thread'ini temizle (QThread: Destroyed fix)
        try:
            if hasattr(self, '_patient_delete_thread') and self._patient_delete_thread is not None:
                thread = self._patient_delete_thread
                self._patient_delete_thread = None
                try:
                    if thread.isRunning():
                        thread.requestInterruption()
                        if not thread.wait(2000):   # 2 saniye bekle
                            thread.terminate()
                            thread.wait(500)
                except (RuntimeError, AttributeError):
                    self.logger.debug("Patient delete thread already deleted")
                finally:
                    self.logger.debug("Hasta silme thread'i temizlendi")
        except Exception as e:
            self.logger.error(f"Hasta silme thread temizlenirken hata: {e}")
