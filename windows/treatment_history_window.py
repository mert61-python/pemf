...
class HistoryLoaderRunnable(QRunnable):
    ...
    def __init__(self, db, start_date, end_date, treatment_mode, logger):
        super().__init__()
        self.db = db
...
class SessionDetailLoaderRunnable(QRunnable):
    ...
    def __init__(self, db, session_id, logger):
        super().__init__()
        self.db = db
...
class TreatmentHistoryWindow(QWidget, StyleMixin):
    ...
    def update_table(self, sessions):
        ...
        for row, session in enumerate(sessions):
            ...
            patient_name = str(session.get('patient_name', '')).strip() or 'Bilinmiyor'
            patient_surname = str(session.get('patient_surname', '')).strip() or ''
    ...
    def update_statistics(self):
        ...
        try:
            ...
            self.total_sessions_label.setText(f"Toplam Seans: {stats.get('total_sessions', 0)}")
            self.monthly_sessions_label.setText(f"Bu Ay: {stats.get('monthly_sessions', 0)}")
            self.avg_duration_label.setText(f"Ort. Süre: {stats.get('average_duration', 0)} dk")
            
        except Exception as e:
            self.logger.error(f"İstatistik güncelleme hatası: {e}", exc_info=True)
    ...
    def generate_selected_pdf(self):
        ...
        if not session_ids:
            QMessageBox.warning(
                self,
                "Hata",
                "Seçili seansların ID'leri alınamadı.",
                QMessageBox.StandardButton.Ok
            )
            return
