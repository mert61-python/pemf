...
class CameraAIThread(QThread):
    ...
    def run(self):
        try:
            ...
            INFERENCE_INTERVAL = 1.0  # 1 Hz (Senkronizasyon İçin)
            ...
            while self._running:
                ...
                if detected and (current_time - last_inference_time >= INFERENCE_INTERVAL):
                    ...
                    try:
                        # KediPredictor.predict() → {D1..D7, P1..P7, result_E}
                        ...
                        D_vals = np.clip(
                            [result[f"D{i}"] for i in range(1, 8)], 0.0, 0.49
                        )
                        P_vals = [result[f"P{i}"] for i in range(1, 8)]
                ...
                # Küçük bekleme (Thread'i rahatlatmak için)
                QThread.msleep(30)
