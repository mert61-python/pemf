...
class KPIDashboardWindow(QMainWindow, StyleMixin):
    ...
    def __init__(self, main_window=None):
        ...
        self._kpi_state_lock = threading.Lock()
        self.sensor_data_updated.connect(
        ...
        for i, (name, unit, definition, purpose, icon, color) in enumerate(kpis):
            ...
            purpose_icon = QLabel("💡")
            purpose_icon.setStyleSheet("font-size: 11pt; margin-right: 8px;")
    ...
    def closeEvent(self, event):
        ...
        # Sinyal bağlantısını kes (Critical Race Condition Fix)
        try:
            self.sensor_data_updated.disconnect()
        except Exception:
            pass
            
        # Ana pencere referansından kpi dashboardu manuel olarak None yap
        try:
            if hasattr(self, 'main_window') and getattr(self.main_window, 'kpi_dashboard_window', None) == self:
                self.main_window.kpi_dashboard_window = None
        except Exception:
            pass
    ...
    def _update_realtime_kpis(self):
        ...
        try:
            ...
            if hasattr(self, 'realtime_energy_label'):
                ...
                if total_energy_wh < 1000:
                    self.realtime_energy_label.setText(f"{total_energy_wh:.2f} Wh")
                else:
                    self.realtime_energy_label.setText(f"{total_energy_wh/1000:.3f} kWh")
            ...
            if self.main_window and hasattr(self.main_window, 'update_kpi_energy'):
                try:
                    self.main_window.update_kpi_energy(total_energy_wh)
                except Exception as e:
