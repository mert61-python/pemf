...
class DigitalTwinFileCopyThread(QThread):
    ...
    def _wait_for_exe_ready(self, exe_path: Path, timeout_ms: int = 8000) -> bool:
        """Kopyalanan EXE dosyasının erişilebilir hale gelmesini bekler."""
        elapsed = 0
        poll_ms = 250
        previous_size = -1
        stable_count = 0
        ...
        while elapsed < timeout_ms:
            if exe_path.exists():
                try:
                    size_now = exe_path.stat().st_size
                    if size_now > 0 and size_now == previous_size:
                        stable_count += 1
                    else:
                        stable_count = 0
                    previous_size = size_now

                    # Boyut iki tur stabil kaldıysa dosyayı hazır kabul et.
                    if stable_count >= 2:
    ...
    def run(self):
        ...
        try:
            ...
            exe_path = self.pemf_temp_dir / "PEMF.exe"
            if not self._wait_for_exe_ready(exe_path):
                raise RuntimeError("PEMF.exe dosyasi belirtilen surede kullanima hazir olmadi")
