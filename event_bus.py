...
class EventBus:
    ...
    def _is_qt_bound_callback(self, callback: Callable) -> bool:
        """Qt QObject bound method ise callback'i Qt-safe kabul et."""
        try:
            if not self._qt_app:
                return False
            ...
            return isinstance(owner, QObject)
        except Exception:
            return False
    ...
    def _call_qt_callback(self, subscription: Subscription, event: Event):
        ...
        try:
                ...
                if current_thread == self._qt_main_thread:
                    ...
                    def execute_callback():
                        try:
                            ...
                            logger.error("Qt callback execution failed: %s", e)
                            self._stats['failed_deliveries'] += 1
            ...
            logger.error("Qt callback failed: %s", e)
            self._stats['failed_deliveries'] += 1
