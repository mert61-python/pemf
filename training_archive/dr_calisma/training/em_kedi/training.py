#!/usr/bin/env python3
"""
training_em_kedi.py — EM Kedi Yeni Dataset Benchmark
==============================================================================
145 DL (70 Raw + 70 Normal + 5 diamond) + sklearn + ensembles

Input (10):  x, y, z + 7 one-hot organ_id (0,1,2,3,4,5,6)
Output (15): D1-D7 (BAGIMSIZ), P1-P7 (0-360), result_E

YENI DATASET OZELLIKLERI:
  - D1-D7 bagimsiz (eski kedi'de hepsi ayniydi)
  - 7 organ (organ4 eklendi, daha dengeli dagilim)
  - P 0-360 tam daire
  - target_B/max_b yok — input olarak kullanilmaz
  - achieved_B degisken (0.00097-0.00103)
  - Shuffle + Stratified split + DataLoader shuffle
"""
...
ORGAN_IDS = [0, 1, 2, 3, 4, 5, 6]   # 7 organ (4 eklendi)
N_ORGANS = len(ORGAN_IDS)

IN_DIM = 3 + N_ORGANS + 7   # x,y,z + 7 one-hot + 7 per-coil B = 17
OUT_DIM = 7 + 14 + 1    # D1-7 + sin(P1-7)+cos(P1-7) + result_E = 22
BS = 8192
LR = 0.002
NUM_WORKERS = 4
...
EPOCH_MAP = {
    ...
    "Large": 500, "XL": 300, "XXL": 300,
}

# FP16 mixed precision (hizli) — sadece buyuk modeller FP32
FP32_MODELS = {"TabNet_XL", "TabNet_XXL", "MLP_XXL", "ResNet_XXL", "FiLM_XXL"}

# v9 entegrasyonu: organ-weighted loss
ORGAN_LOSS_W = {0: 0.5, 1: 1.0, 2: 1.5, 3: 1.0, 4: 2.0, 5: 1.2, 6: 1.2}
LAMBDA_PHASE = 0.5    # phase (sin/cos) loss agirligi — v9 ile ayni denge
LAMBDA_E = 0.1        # energy loss agirligi
LAMBDA_DUTY = 0.001   # duty ortalamasina ceza
LAMBDA_MAX = 0.01     # duty ust sinir cezasi (v9'dan)
WARMUP_EPOCHS = 20    # warmup epoch sayisi
...
# ============================================================
print("=" * 80)
print(f"EM_KEDI EGITIM SETI YUKLENIYOR... (Device: {DEVICE})")
print(f"IN_DIM={IN_DIM}, OUT_DIM={OUT_DIM}, BS={BS}, LR={LR}")
print("=" * 80)

CSV_PATH = "/home/user02/dr/dataset/em_kedi/kedi_egitim_seti.csv"
...
# organ_id float → int, filtre
df["organ_id"] = df["organ_id"].astype(int)
...
e_col = "result_E"
# y_cols sadece referans — gercek y_raw asagida sin/cos ile olusturulur

# ---- One-hot encode organ_id (7 organ) ----
...
# ---- Per-coil B katkisi (manyetik_matris.npz'den) ----
print(f"\n[Per-coil B] manyetik_matris.npz yukleniyor...")
npz_path = "/home/user02/dr/dataset/em_kedi/manyetik_matris.npz"
npz_data = np.load(npz_path)
B_matrix = npz_data['B_matrix']    # (7, 101, 156780, 3)
npz_coords = npz_data['coords']    # (156780, 3)

# Her bobinin her noktadaki max B buyuklugu
B_per_coil_all = np.zeros((len(npz_coords), 7), dtype=np.float32)
for coil in range(7):
    B_mag = np.sqrt(np.sum(B_matrix[coil]**2, axis=-1))  # (101, 156780)
    B_per_coil_all[:, coil] = B_mag.max(axis=0)           # (156780,)
del B_matrix, npz_data  # memory free

# KDTree ile em_kedi koordinatlarini npz'ye esle
from scipy.spatial import KDTree
tree = KDTree(npz_coords)
em_coords = df[['x','y','z']].values
dists, idx = tree.query(em_coords)
B_per_coil = B_per_coil_all[idx]  # (N, 7)
print(f"  Per-coil B shape: {B_per_coil.shape}")
print(f"  Eslesme mesafesi: mean={dists.mean():.2f}mm, max={dists.max():.2f}mm")
for coil in range(7):
    print(f"    Bobin {coil+1}: min={B_per_coil[:,coil].min():.6f} max={B_per_coil[:,coil].max():.6f}")
del B_per_coil_all, npz_coords, tree

# ---- Build X (17: x,y,z + 7 one-hot + 7 per-coil B) and y (22) ----
...
# Phase DIFFERENCE encoding: ΔP_i = P_i - P_1 (gauge freedom kalkar)
# P1 referans olarak kalır, P2-P7 fark olarak encode edilir
p_angles_deg = df[p_cols].values          # (N, 7) degrees
p1_ref = p_angles_deg[:, 0:1]            # P1 referans (N, 1)
p_diff = p_angles_deg[:, 1:] - p1_ref    # ΔP2-ΔP7 (N, 6)
# Wrap to [-180, 180]
p_diff = p_diff - 360.0 * np.round(p_diff / 360.0)

# Sin/cos encoding: P1 + 6 ΔP = 7 açı → 14 sin/cos
p_all_deg = np.column_stack([p1_ref, p_diff])  # (N, 7): P1, ΔP2, ..., ΔP7
p_rad = np.deg2rad(p_all_deg)
p_sin = np.sin(p_rad)
p_cos = np.cos(p_rad)

# y = [D1-7, sin(P1,ΔP2-7), cos(P1,ΔP2-7), E] = 22 outputs
y_d = df[d_cols].values
y_e = df[e_col].values.reshape(-1, 1)
y_raw = np.column_stack([y_d, p_sin, p_cos, y_e]).astype(np.float32)
print(f"X_raw shape: {X_raw.shape} | y_raw shape: {y_raw.shape}")
print(f"  D(7) + sin(P1+ΔP2-7)(7) + cos(P1+ΔP2-7)(7) + E(1) = {y_raw.shape[1]} outputs")
print(f"  Phase difference encoding: P1 referans, ΔP2-ΔP7 fark")

# Keep original values for metric computation
y_d_orig = y_d
y_e_orig = df[e_col].values
...
print(f"D bagimsiz mi: {(df[d_cols].nunique(axis=1)==1).sum()==0}")
...
# ============================================================
# 2. Scaling — SADECE numeric feature'lar scale edilir
#    x,y,z (0:3) → StandardScaler
#    organ_oh (3:10) → olduğu gibi (0/1 binary)
#    B_coil (10:17) → StandardScaler
#    y → StandardScaler (hepsi numeric)
# ============================================================
# X: numeric index'leri
numeric_x_idx = list(range(0, 3)) + list(range(3 + N_ORGANS, IN_DIM))  # x,y,z + B_coil
binary_x_idx = list(range(3, 3 + N_ORGANS))  # organ one-hot

scaler_X = StandardScaler()
# Sadece numeric kolonlari fit et
X_train_numeric = X_train_raw[:, numeric_x_idx]
scaler_X.fit(X_train_numeric)

def scale_X(X_raw):
    """Numeric kolonlari scale et, one-hot olduğu gibi birak."""
    X_out = X_raw.copy()
    X_out[:, numeric_x_idx] = scaler_X.transform(X_raw[:, numeric_x_idx])
    return X_out.astype(np.float32)

X_train_sc = scale_X(X_train_raw)
X_val_sc = scale_X(X_val_raw)
X_test_sc = scale_X(X_test_raw)

# y: hepsi numeric, StandardScaler
...
# scale_X fonksiyonunu da kaydet (inference icin)
joblib.dump({"numeric_idx": numeric_x_idx, "binary_idx": binary_x_idx},
            os.path.join(MODELS_DIR, "scaler_X_idx.pkl"))
print(f"\nScaler: numeric {len(numeric_x_idx)} kolon scale, {len(binary_x_idx)} kolon one-hot olduğu gibi")

# ============================================================
# 3. DataLoader + WeightedRandomSampler (organ dengesi) + organ_id tensor
# ============================================================
train_organ_ids = df.iloc[idx_train]["organ_id"].values.astype(int)
val_organ_ids = df.iloc[idx_val]["organ_id"].values.astype(int)
test_organ_ids_arr = df.iloc[idx_test]["organ_id"].values.astype(int)

train_ds = TensorDataset(torch.FloatTensor(X_train_sc), torch.FloatTensor(y_train_sc), torch.LongTensor(train_organ_ids))
val_ds = TensorDataset(torch.FloatTensor(X_val_sc), torch.FloatTensor(y_val_sc), torch.LongTensor(val_organ_ids))
test_ds = TensorDataset(torch.FloatTensor(X_test_sc), torch.FloatTensor(y_test_sc), torch.LongTensor(test_organ_ids_arr))

# WeightedRandomSampler: organ dagilimini dengele
organ_counts = np.bincount(train_organ_ids, minlength=len(ORGAN_IDS))
organ_weights = 1.0 / (organ_counts + 1e-5)
sample_weights = organ_weights[train_organ_ids]
train_sampler = WeightedRandomSampler(weights=sample_weights, num_samples=len(sample_weights), replacement=True)

train_loader = DataLoader(train_ds, batch_size=BS, sampler=train_sampler, num_workers=NUM_WORKERS, pin_memory=True)
...
# Organ loss weights tensor (GPU'ya yükle)
organ_weights_tensor = torch.tensor(
    [ORGAN_LOSS_W.get(i, 1.0) for i in range(len(ORGAN_IDS))],
    device=DEVICE, dtype=torch.float32
)
print(f"WeightedRandomSampler aktif | Organ loss weights: {ORGAN_LOSS_W}")
...
class CNN1DModel(nn.Module):
    def __init__(self, channels, dropout=0.2, out_dim=OUT_DIM, in_dim=IN_DIM):
        ...
        fc_layers = [nn.Linear(channels[-1], 128), nn.Mish()]
...
class RNNModel(nn.Module):
    def __init__(self, rnn_type="lstm", hidden_size=128, num_layers=2, dropout=0.2, bidirectional=False, out_dim=OUT_DIM, in_dim=IN_DIM):
        ...
        self.n_steps = 8
        self.step_dim = 32
        self.proj = nn.Sequential(nn.Linear(in_dim, self.n_steps * self.step_dim), nn.Mish())
        ...
        fc_l = [nn.Linear(fc_in, 64), nn.Mish()]
...
class HybridCNNRNN(nn.Module):
    def __init__(self, cnn_ch=[32,64], rnn_type="lstm", hs=128, nl=1, do=0.2, bidir=False, out_dim=OUT_DIM, in_dim=IN_DIM):
        ...
        hfc = [nn.Linear(hs*(2 if bidir else 1), 64), nn.Mish()]
...
class TabNetBlock(nn.Module):
    def __init__(self, in_dim, hd, out_dim, ns=3, do=0.2):
        ...
        self.fcs = nn.ModuleList([
            nn.Sequential(nn.Linear(in_dim, hd), nn.BatchNorm1d(hd), nn.Mish(), nn.Dropout(do)) if do > 0
            else nn.Sequential(nn.Linear(in_dim, hd), nn.Mish())
            for _ in range(ns)])
...
# ============================================================
# FiLM ResNet (v9 mimarisi — organ-specific conditioning)
# ============================================================
class FiLMLayer(nn.Module):
    ...
    def forward(self, x, class_id):
        return x * self.gamma(class_id) + self.beta(class_id)
...
class FiLMResBlock(nn.Module):
    ...
    def forward(self, x, org_id):
        res = x
        x = self.act1(self.film1(self.lin1(x), org_id))
        x = self.act2(self.film2(self.lin2(x), org_id))
        return x + res

class EAlanAI_FiLM(nn.Module):
    """v9 FiLM ResNet: input=coords(3), organ_id ayrı conditioning."""
    def __init__(self, coord_dim=3, out_dim=OUT_DIM, hidden=512, num_classes=7, n_blocks=8, in_dim=None):
        super().__init__()
        self.coord_dim = coord_dim
        self.in_proj = nn.Linear(coord_dim, hidden)
        self.blocks = nn.ModuleList([FiLMResBlock(hidden, num_classes) for _ in range(n_blocks)])
        self.out_proj = nn.Linear(hidden, out_dim)
    def forward(self, x):
        # x: (B, IN_DIM) — ilk 3 = coords, index 3-9 = organ one-hot
        coords = x[:, :3]
        # One-hot → organ_id int (argmax)
        organ_oh = x[:, 3:3+N_ORGANS]
        org_id = organ_oh.argmax(dim=1)
        h = self.in_proj(coords)
...
def create_model(cfg, out_dim=OUT_DIM, in_dim=IN_DIM):
    ...
    if t == "film": return EAlanAI_FiLM(coord_dim=3, out_dim=out_dim, hidden=cfg["hidden"], num_classes=N_ORGANS, n_blocks=cfg["blocks"], in_dim=in_dim)
...
# 6. CSV
# ============================================================
_CSV = os.path.join(MODELS_DIR, "model_kedi_karsilastirma.csv")
...
def inverse_transform_predictions(pred_sc):
    """Inverse transform (N,22) scaled → (D 7, P_deg 7, E 1).
    Phase difference: P1 referans, ΔP2-7 → absolute P2-7 = P1 + ΔP_i"""
    pred_real = scaler_y.inverse_transform(pred_sc)
    d_pred = pred_real[:, :7]
    p_sin = pred_real[:, 7:14]   # sin(P1), sin(ΔP2), ..., sin(ΔP7)
    p_cos = pred_real[:, 14:21]  # cos(P1), cos(ΔP2), ..., cos(ΔP7)
    # atan2 → derece: P1 ve ΔP2-7
    p_angles = np.rad2deg(np.arctan2(p_sin, p_cos))  # (N, 7)
    # Phase diff → absolute: P_i = P1 + ΔP_i
    p1_ref = p_angles[:, 0:1]           # P1 referans (N, 1)
    p_diffs = p_angles[:, 1:]           # ΔP2-ΔP7 (N, 6)
    p_absolute = np.column_stack([p1_ref, p1_ref + p_diffs])  # (N, 7)
    p_pred = p_absolute % 360.0
    e_pred = pred_real[:, 21]
...
def train_dl(cfg, name, epochs):
    ...
    clip_norm = 0.5 if not use_fp16 else 1.0
    _lr = cfg.get("lr", LR)
    ...
    if os.path.exists(pt_path):
        ...
        opt = torch.optim.AdamW(mdl.parameters(), lr=1e-5, weight_decay=1e-4)

        # Warmup + Cosine Annealing scheduler
        def _get_lr(epoch):
            if epoch < WARMUP_EPOCHS:
                return _lr * (epoch + 1) / WARMUP_EPOCHS
            else:
                progress = (epoch - WARMUP_EPOCHS) / max(epochs - WARMUP_EPOCHS, 1)
                return 1e-5 + 0.5 * (_lr - 1e-5) * (1 + np.cos(np.pi * progress))
        sched = torch.optim.lr_scheduler.LambdaLR(opt, lr_lambda=lambda ep: _get_lr(ep) / 1e-5)

        # Custom loss: organ-weighted + phase-specific + duty penalty
        def crit(y_pred, y_true, org_ids=None):
            # Output layout: D(0:7) + sinP(7:14) + cosP(14:21) + E(21)
            # Per-sample organ weight
            if org_ids is not None:
                w = organ_weights_tensor[org_ids].unsqueeze(1)  # (B, 1)
            else:
                w = 1.0

            loss_D = (nn.functional.mse_loss(y_pred[:, :7], y_true[:, :7], reduction='none') * w).mean()
            loss_sinP = (nn.functional.mse_loss(y_pred[:, 7:14], y_true[:, 7:14], reduction='none') * w).mean()
            loss_cosP = (nn.functional.mse_loss(y_pred[:, 14:21], y_true[:, 14:21], reduction='none') * w).mean()
            loss_E = (nn.functional.mse_loss(y_pred[:, 21:], y_true[:, 21:], reduction='none') * w).mean()
            loss_duty_penalty = torch.mean(torch.abs(y_pred[:, :7]))
            loss_max_penalty = torch.mean(nn.functional.relu(y_pred[:, :7] - 3.0))
            return (loss_D
                    + LAMBDA_PHASE * (loss_sinP + loss_cosP)
                    + LAMBDA_E * loss_E
                    + LAMBDA_DUTY * loss_duty_penalty
                    + LAMBDA_MAX * loss_max_penalty)
        ...
        for ep in range(start_epoch, epochs+1):
            ...
            for batch in _train_loader:
                ...
                ob = batch[2].to(DEVICE, non_blocking=True) if len(batch) > 2 else None
                opt.zero_grad()
            ...
            with torch.no_grad(), torch.cuda.amp.autocast(enabled=use_fp16):
                for batch in _val_loader:
                    ...
                    yb = batch[1].to(DEVICE, non_blocking=True)
                    ob = batch[2].to(DEVICE, non_blocking=True) if len(batch) > 2 else None
                    vl_ += crit(mdl(xb), yb, ob).item()*len(xb)
            vl_ /= len(val_ds)
            sched.step()  # Warmup + Cosine Annealing
...
split_models = {
    ...
    # FiLM ResNet (v9 mimarisi — organ-specific conditioning)
    # ============================================================
    "FiLM_Small":  {"type": "film", "hidden": 256, "blocks": 4},
    "FiLM_Medium": {"type": "film", "hidden": 512, "blocks": 6},
    "FiLM_Large":  {"type": "film", "hidden": 512, "blocks": 8},   # v9 orijinal
    "FiLM_XL":     {"type": "film", "hidden": 768, "blocks": 8},
    "FiLM_XXL":    {"type": "film", "hidden": 1024, "blocks": 10},
}
...
print(f"\n{'='*80}\nSKLEARN - {len(sk_list)} MODEL (joint 15-output, StandardScaler X+y)\n{'='*80}")
...
config = {
    "seed": SEED,
    "approach": "trainai-optimized + shuffle + stratified + one-hot organ",
    "in_dim": IN_DIM,
    "out_dim": OUT_DIM,
    "epoch_map": EPOCH_MAP,
    "batch_size": BS,
    ...
    "preprocessing": {
        ...
        "y_scaler": "StandardScaler (joint D+P+E)",
