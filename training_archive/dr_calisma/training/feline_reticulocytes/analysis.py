#!/usr/bin/env python3
"""
analysis_petri_dish.py — YOLO Feline Reticulocytes Post-Analysis
================================================================
...
CSV_PATH = os.path.join(MODELS_DIR, "model_feline_ret_karsilastirma.csv")
...
def _s1_model_comparison():
    ...
    # Box
    ax = axes[0]
    ax.bar(x - w/2, df["mAP50"], w, label="mAP50", color="#2196F3", edgecolor="black")
    ax.bar(x + w/2, df["mAP50_95"], w, label="mAP50-95", color="#FF9800", edgecolor="black")
    ax.set_xticks(x)
    ax.set_xticklabels([m.replace("yolo26","Y26-").replace("","") for m in models], rotation=15)
    ax.set_ylabel("Score")
    ...
    # Mask
    ax = axes[1]
    ax.bar(x - w/2, df["mAP50"], w, label="mAP50", color="#4CAF50", edgecolor="black")
    ax.bar(x + w/2, df["mAP50_95"], w, label="mAP50-95", color="#E91E63", edgecolor="black")
    ax.set_xticks(x)
    ax.set_xticklabels([m.replace("yolo26","Y26-").replace("","") for m in models], rotation=15)
    ...
    plt.suptitle("YOLO26 Model Comparison — Detection Performance", fontsize=14, y=1.02)
    plt.tight_layout()
...
def _s2_speed_accuracy():
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ax.scatter(r["Inference_ms"], r["mAP50_95"],
                   s=r["Params_M"] * 20, color=colors[i], edgecolor="black",
                   zorder=5, alpha=0.8)
        ax.annotate(r["Model"].replace("yolo26","Y26-").replace("",""),
                     (r["Inference_ms"], r["mAP50_95"]),
...
def _s3_training_curves():
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        try:
            ...
            color = cmap(i)
            label = name.replace("yolo26", "Y26-").replace("", "")
...
def _s4_params_vs_performance():
    ...
    models = [m.replace("yolo26","Y26-").replace("","") for m in df["Model"]]
    x = np.arange(len(models))
    ...
    ax2 = ax1.twinx()
    ax2.plot(x, df["mAP50_95"], "o-", color="#E91E63", linewidth=2, markersize=8, label="mAP50-95")
    ax2.plot(x, df["mAP50"], "s--", color="#4CAF50", linewidth=2, markersize=8, label="Mask mAP50")
...
def _s5_comprehensive_radar():
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        values = [r["mAP50"], r["mAP50_95"], r["mAP50"],
                  r["mAP50_95"], r["F1"], r["F1"]]
        values += values[:1]
        label = r["Model"].replace("yolo26", "Y26-").replace("", "")
