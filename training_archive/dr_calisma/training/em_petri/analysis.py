#!/usr/bin/env python3
"""
analysis_petri.py — Post-Training Analizleri (SCI Makale)
==========================================================
training_kedi.py'yi calistirdiktan sonra bu script'i calistir.

Veri kaynaklari:
  - models_petri/predictions.npz       (tum tahminler, labellar)
  - models_petri/model_petri_karsilastirma.csv  (metrikler)
  - models_petri/training_curves/*.csv (loss egrileri)
  - models_petri/pareto_data.csv       (complexity vs performance)
  - models_petri/*.pt, *.pkl           (modeller - noise/perm importance icin)
...
SEED = 42
ORGAN_IDS = [0, 1]
FEATURE_NAMES = ["x", "y", "z", "target_B"] + [f"organ_{o}" for o in ORGAN_IDS]
IN_DIM = 6
OUT_DIM = 15
...
print("=" * 80)
print("ANALYSIS_PETRI.PY — Petri SCI Post-Analizleri")
print("=" * 80)

# ============================================================
# Load predictions + results
# ============================================================
print("\n[LOAD] predictions.npz + model_petri_karsilastirma.csv ...")
PRED_PATH = os.path.join(MODELS_DIR, "predictions.npz")
CSV_PATH = os.path.join(MODELS_DIR, "model_petri_karsilastirma.csv")
...
for k in preds_npz.files:
    if k.endswith("__test"):
        all_test_preds[k[:-6]] = preds_npz[k]
    elif k.endswith("__val"):
        all_val_preds[k[:-5]] = preds_npz[k]

print(f"  {len(all_test_preds)} modelin tahminleri yuklendi")
...
def get_errors(model_name):
    ...
    d_err = np.mean((y_d_test - p[:, :7])**2, axis=1)
    p_err_raw = circ_diff(yp_test_deg, p[:, 7:14])
    p_err = np.mean(p_err_raw**2, axis=1)
    e_err = (y_e_test - p[:, 14])**2
    ...
    for nm in sk_models_available[:10]:
        ...
        for feat_idx, feat_name in enumerate(FEATURE_NAMES):
            ...
            for rep in range(n_repeats):
                ...
                d_p = y_perm_orig[:, :7]; p_p = y_perm_orig[:, 7:14]; e_p = y_perm_orig[:, 14]
        ...
        try:
            ...
            for variant, path in [("FP32", onnx_path), ("INT8", int8_path)]:
                try:
                    ...
                    d_p = y_orig[:, :7]; p_p = y_orig[:, 7:14]; e_p = y_orig[:, 14]
                    d_r2 = float(r2_score(y_d_test, d_p))
                    p_cr2 = float(circ_r2(yp_test_deg, p_p))
                    e_r2 = float(r2_score(y_e_test, e_p))
