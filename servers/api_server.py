...
async def _update_status():
    """Kurulu sürüm vs GitHub 'exe' branch latest.json — UI bildirimi için. (Yerel muaf / uzak token — normal auth.)"""
    try:
        from servers.update_manager import get_status
        return get_status()
    except Exception:
        logging.exception("update status hatasi")
        return {"checked": False, "available": False, "error": "update_manager yok"}


@app.post("/api/update/apply")
async def _update_apply(request: Request):
    """Operatör ONAYI → installer'ı indir + SHA256 doğrula + AKTİF TEDAVİ YOKKEN sessiz kur (blocking → thread)."""
    try:
        from servers.update_manager import apply_update
        return await asyncio.to_thread(apply_update)
    except Exception:
        logging.exception("update apply hatasi")
        return Response(status_code=500, content='{"detail":"guncelleme uygulanamadi"}', media_type="application/json")
