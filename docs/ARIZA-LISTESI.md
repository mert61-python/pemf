...
### ✅ A2 · MD5 birebir kopya CV kodu — **BİRLEŞTİRİLDİ (2026-09-19)**
| | |
|---|---|
...
| **Eski karar (2026-09-17)** | **ERTELENDİ.** Şimdi birleştirmek, yeniden yazılacak geometriyi birleştirmek olur. Doğru an: rotasyon düzeltmesi. ⚠️ Doğrulaması `ai_hub` PYZ dışında sevk edildiği için **backend build** ister |
| **YAPILDI (2026-09-19, sahip isteği)** | Ortak gövde **`ai_hub/cv_ortak/`**a alındı (`git mv` ile, geçmiş korunarak); profil paketlerinde **ince kabuk** kaldı — kabuklar gövdenin %15-36'sı. `CabinConfig` artık iki profilde **aynı sınıf nesnesi**, yani ayrışması fiziksel olarak mümkün değil.<br>⚠️ **KAPSAM ÖLÇÜLEREK DAR TUTULDU:** iki pakette `pipeline.py` (498 satır fark), `color_segment.py` (590), `render.py` (357), `cli.py` (198), `api.py` (8) de var ama bunlar **gerçek profil-başına uygulamalar**, kopya değil. `inference_cat_organ/lib/cabin_config.py` de (185 satır) ayrı bir modül — adı benzer, içeriği değil. Birleştirilen tam 3 dosya.<br>⚠️ **İKİ GERÇEK BULGU — ikisi de birleştirme sırasında ölçüldü:**<br>**(1) Yakın kaza.** `cabin_config.py` içinde `DEFAULT_CONFIG_YAML = Path(__file__).parent / "cabin_config_example.yaml"` vardı. Modül taşınınca bu yol `cv_ortak/`a kayar ve orada örnek YAML **yoktur**; üstelik iki profilin örneği FARKLIDIR (192 vs 206 satır). `load_cabin_config(None)` ÜRETİMDE **dört yerden** çağrılıyor (`ai_router.py:985,1009` · `predictors.py:104,115`) → naif taşıma dördünü de `FileNotFoundError` ile kırardı. Varsayılan artık **çağırandan** gelir; kabuk kendi `_DIR`ini geçirir. *(Aynı sınıfı bu depo aynı gün `bootstrap.ps1` taşımasında da yaşadı — taşınan şeyin KENDİ konum mantığı.)*<br>**(2) Sabit MQTT `client_id`.** Ortak yayıncı yapılandırmadan gelen kimliği doğrudan `mqtt.Client(client_id=...)`a veriyordu. Kayıtlı karar: *client_id SABİT OLAMAZ* — aynı kimlikle ikinci bağlantı brokerda birincisini düşürür. Backend bunu **iki denetim turunda** `_mqtt_client_id(role)` ile çözmüştü; CV yayıncısı o dersten **muaf kalmıştı** (hem kod varsayılanı hem örnek YAML sabit dize veriyordu). Artık `benzersiz_client_id()` süreç eki koyuyor, **23 karakter** sınırı korunarak (`estop-cloud` rolünde tam bu sınır aşılmıştı).<br>**Kapı:** `tests/test_cv_ortak_tek_uygulama.py` (20 test) · **7 mutasyon**, her biri farklı testi vurdu.<br>⚠️ **MUTASYON KAPININ İKİ KUSURUNU BULDU.** (a) `test_..._client_id_KIMLIGI_korundu` **vakumdu**: örnek YAML `client_id`i açıkça yazdığı için varsayılan dalına HİÇ düşülmüyordu; profil sabitini değiştiren mutasyon yeşil geçti. Artık YAML'dan alan silinip varsayılan dalı **gerçekten koşturuluyor**. (b) Kopya taraması `git ls-files` kullanıyordu — git yalnız **izlenen** dosyaları listeler, yeni kabuklar takipsizdi ve taramaya HİÇ girmiyorlardı; "kopyayı geri koy" mutasyonu bu yüzden görülmedi. Tarama **dosya sistemine** çevrildi.<br>**Devralınan iki test güncellendi (niyet korunarak):** `test_KRITIK_iki_kopya_BIREBIR_ayni` → `test_KRITIK_iki_profil_AYNI_uygulamayi_kullanir` — kendi docstring'i *"tek kaynak yapılana kadar"* diyordu; ölçüt artık **fonksiyon nesnesi kimliği** (dosya karşılaştırması iki kabukta yanıltıcı olurdu). Ayrıştırıcı çıpası tek kaynağa taşındı; YAML'lar profil-başına ayrı ölçülmeye devam ediyor.<br>**ÜRÜN DOĞRULAMASI:** tam süit **3176 geçti / 0 düşen** · ruff `--no-cache` temiz · backend EXE **yeniden derlendi**: kod koruması **67 → 70** modül (0 başarısız), PYZ koruma kapısı ✓, sevk ağacı **70/70** ✓, AI hazırlık **16/16** ✓, çalışma-anı koruma 16/16 ✓ · sevk ağacında gövdeler `cv_ortak`ta (130/98/120 KB), profil kabukları **18,9/18,9/43 KB** = gövdenin %15-36'sı → kopya değil · **48/48 ürün senaryosu** yeni artefaktta geçti.<br>⚠️ **EXE sha'sı DEĞİŞMEDİ ve bu DOĞRU:** `ai_hub` PYZ'ye girmiyor (koruma kapısı öyle diyor), `_internal/` altında ayrı sevk ediliyor. İlk bakışta "bayat ikili" sanıldı; ölçüm yanlış artefakta bakıyordu. Değişen `_internal/ai_hub/` ağacı (21:12'de yeniden üretildi, `cv_ortak/` daha önce hiç yoktu). |
| **Kim** | Ben |
...
| **ÜRÜN DOĞRULAMASI (2026-09-19 15:58)** | Backend EXE **yeniden derlendi** ve düzeltme **sevk edilen artefaktta** ölçüldü.<br>⚠️ **Metin araması YAPILMADI** — PYZ sıkışık olduğu için ham EXE'de metin aramak sahte "YOK" verir (1.9.50 kaydı). `CArchiveReader` + `marshal` ile **kod nesnesi** çözüldü.<br>⚠️ **Araç önce kanıtlandı:** (a) bilinen üç modülü (`servers.live_state` · `utils.path_utils` · `database.db_hatalari`) çözebildiği gösterildi; (b) **aynı araç ESKİ EXE'de koşuldu ve tam bu üç maddeye KIRMIZI dedi** → negatif kontrol, yani yeşil "görünüş" değil fark ölçümü.<br>**Sonuçlar:** `utils.dongu_guvenligi` PYZ TOC'unda **VAR** (TOC 11045 → **11046**, tam +1) · `servers.api_server` donmuş bytecode'u `_dongu_hatasini_bildir`e **atıf yapıyor** · `servers.efield_live` aynı şekilde · beş döngü mesajı da sevk edilen bytecode'da duruyor.<br>**Bayat ikili kapısı:** exit 0'a GÜVENİLMEDİ (bu depoda başarısızken de exit 0 görüldü) — mtime `13:45:32 → 15:58:42`, sha256 `664c2579… → c9b183ac…`, boyut `56756441 → 56758655`.<br>**Build kapıları:** headless guard ✓ · kod koruması 67/67 ✓ · PYZ koruma kapısı ✓ · sevk ağacı 67/67 ✓ · AI hazırlık **16/16** ✓ · çalışma-anı koruma 16/16 ✓.<br>**ÇALIŞMA-ANI:** donmuş EXE başlatıldı → `/api/health` **200** (`status:online`, `core_initialized:true`, `dbReady:true`) → `/api/dashboard-snapshot` 6 sn arayla iki kez örneklendi, **8/8 bobin değişti** → yeniden bağlanan altı döngüden biri (`hardware-sim`) donmuş süreçte **gerçekten dönüyor**. Koşum log'unda traceback YOK. Süreç temiz kapatıldı (aktif seans yoktu). |
| **Kim** | Ben |
...
| ✅ **C3** | **KAPANDI 2026-09-19 — SİLİNECEK DİZİN YOK.** Üçüncü ölçüm, üçüncü kez liste yanlış çıktı. 2026-09-17'de altı adayın beşi CANLI ölçülmüştü (`frontend/`→`app.mount("/")` ürünün ana arayüzü · `dema-terapi-simülatörü/`→`/simulator` + spec datas · `web_static/`→spec datas · `lattekurulum/`→`build_installer.ps1` VC++ redist · `website/`→kendi README'si "silme, bırak" kararını taşıyor) ve `tests/test_silinecek_sanilan_dizinler_CANLI.py` yazılmıştı. **2026-09-19'da yedinci aday `ai/` ölçüldü ve bu satırın kendi iddiası ÜÇ yerden yanlış çıktı:** (1) `ai/config.py` 349 değil **376** satır — iddia yazıldığında bile bayattı; (2) `ai/hybrid_recommender.py` ÜRETİMDEN çağrılıyor → `apps/backend/servers/api_server.py:2451`; (3) `config.py` "sıfır import" değil, `ai/__init__.py:23` → `from . import config` ile **yükleniyor** — doğru ifade "sembolleri üretimde KULLANILMIYOR". ⚠️ Neden kaçmış: üretimdeki import **fonksiyon içinde** (8 boşluk girintili), modül-başı tarayan bir ölü-kod aracı GÖRMEZ. Koruma bugüne kadar TESADÜFİdi (bir test paketi modül düzeyinde import ediyordu); kapıya 3 test + tanı testi eklendi · **4 mutasyon** |
