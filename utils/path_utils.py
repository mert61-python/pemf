    ...
    for real_name, template_name in db_files.items():
        ...
        try:
            if source_template.exists():
                print(f"İlk kurulum: {real_name} oluşturuluyor...")
                shutil.copy2(source_template, target_db_path)
            else:
                print(f"UYARI: {template_name} bulunamadı! SQLite otomatik boş dosya oluşturacak.")
        except Exception as e:
            print(f"{real_name} oluşturma hatası: {e}")
