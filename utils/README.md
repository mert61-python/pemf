...
| `panel_yayin.py` | **Çok-panelli AI yanıtı TEK KAYNAK** — fantom/petri boru hatları 7 panel üretiyor (`render_panels`); 2026-09-12 öncesi yalnız `07_combined` mozaiği yayınlanıyor, diğer **6'sı aynı satırda çöpe atılıyordu**. `panelleri_yayina_hazirla()` panel adı + sıra + kapak/kalite sözleşmesini tek yerde tutar; **dört** çağrı yeri (`servers/ai_router.py` fantom/petri + `ai_service/app.py` fantom/petri). ⚠️ Kök `image_base64` KALIR (`paneller` yalnız EK alan) → eski istemci etkilenmez. ⚠️ Petri AYRI ad tablosu kullanır (`02_yolo_dets`/`03_yolo_masks`). ⚠️ `servers/` DEĞİL `utils/`: GPU imajı `servers/`i kopyalamaz — orada kalsaydı panel yayını GPU profilinde sessizce ölürdü. Yaprak modül (yalnız stdlib) — `docker/Dockerfile.ai` imajına tek satırla girer. Kapı: `test_ai_panel_yayini.py` |
