...
def _load_cat_sound():
    ...
    return CatSoundClassifier(
        model_path=_mp("ai_hub/inference_cat_sound/EfficientNet_Lite0.onnx"), runtime="onnx", device=gpu.torch_device()
    )
...
def _load_kidney_ct():
    ...
    return KidneyCTDetector(
        model_path=_mp("ai_hub/inference_human_kidney_ct/yolov8s.onnx"), backend="onnx", device=gpu.torch_device()
    )
...
def _load_thermal():
    ...
    return CatThermalPredictor(model_path=_mp("ai_hub/cat_thermal/GhostNetV2.onnx"), providers=gpu.onnx_providers())
...
def _load_em_petri():
    ...
    return {"cfg": cfg, "predictor": predictor, "yolo": warm.yolo, "yolo_path": yolo_path, "cls": PetriCvPipeline}
...
REGISTRY = {
    ...
    "em_kedi": {
        "loader": _load_em_kedi,
        "kind": "json",
        "title": "AI Pro Bobin Sürüş (em_kedi BiLSTM: x/y/z/organ → D/P/E)",
    },
}
